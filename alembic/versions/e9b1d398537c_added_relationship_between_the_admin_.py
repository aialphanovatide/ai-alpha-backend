...
def upgrade() -> None:
    ...
    if 'tokens' in inspector.get_table_names():
        # Get existing foreign keys for 'tokens'
        foreign_keys = inspector.get_foreign_keys('tokens')
        
        # Check if the foreign key to 'admins' already exists
        fk_exists = any(
            fk['referred_table'] == 'admins' and
...
def downgrade() -> None:
    ...
    # Ensure 'tokens' table exists before trying to drop constraints
    if 'tokens' in inspector.get_table_names():
        foreign_keys = inspector.get_foreign_keys('tokens')
